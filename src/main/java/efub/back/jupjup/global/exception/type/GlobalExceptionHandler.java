...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ErrorResponse> accessDeniedException(AccessDeniedException e) {
		log.info(String.format("Access Denied Exception : %s", e));
		final ErrorResponse response = ErrorResponse.builder()
			.status(HttpStatus.FORBIDDEN)
			.code(ExceptionType.ADMIN_ACCESS_DENIED_EXCEPTION.getErrorCode())
			.message(ExceptionType.ADMIN_ACCESS_DENIED_EXCEPTION.getMessage())
			.build();
		return ResponseEntity.status(response.getStatus()).body(response);
	}
